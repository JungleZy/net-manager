<template>
  <div class="agent-download-container pt-[12px]">
    <a-alert
      message="探针说明"
      description="探针程序用于采集设备信息并上报到管理服务器。请根据您的操作系统选择对应的版本下载。"
      type="info"
      show-icon
      class="mb-4"
    />
    ...
    <a-row :gutter="[12, 12]">
      ...
      <a-col :xs="24" :sm="24" :md="12" :lg="12">
        <a-card class="platform-card" hoverable>
          <template #cover>
            <div class="platform-icon-wrapper">
              <WindowsOutlined class="platform-icon" />
            </div>
          </template>
          <a-card-meta title="Windows 平台">
            <template #description>
              <div class="platform-description">
                <p>适用于 Windows 操作系统</p>
                ...
                <a-space direction="vertical" :size="8" style="width: 100%">
                  <a-button
                    ...
                    block
                    class="layout-center"
                    @click="downloadAgent('net-manager-client-win-x64.exe')"
                  >
                  ...
                  <a-button
                    block
                    class="layout-center"
                    @click="downloadAgent('net-manager-client-win-x86.exe')"
                  >
            ...
            <div class="platform-icon-wrapper">
              ...
              >
                <path
                  ...
                  data-spm-anchor-id="a313x.search_index.0.i1.6d143a81uIsA9z"
                  class="selected"
                  fill="#ffffff"
                ></path>
              </svg>
              ...
              <div class="platform-description">
                ...
                <a-space direction="vertical" :size="8" style="width: 100%">
                  <a-button
                    ...
                    block
                    class="layout-center"
                    @click="downloadAgent('net-manager-client-linux-aarch64')"
                  >
                    <DownloadOutlined /> 下载 aarch64 版本
                  </a-button>
                  <a-button
                    block
                    class="layout-center"
    ...
    <!-- 使用说明 -->
    <div class="w-full" style="padding-top: 24px">
      <h1 style="margin: 0; font-size: 15px">使用说明</h1>
    </div>
    <a-tabs>
...
1. 下载对应版本的探针程序
2. 双击运行 net-manager-client-win-x64.exe（或 x86 版本）
3. 探针将自动连接到管理服务器并开始采集数据
4. 探针启动后会在后台运行，同时自动加入开机自启动</pre
          >
        </a-typography-paragraph>
      </a-tab-pane>
      <a-tab-pane key="linux" tab="Linux">
        <a-typography-paragraph>
          <pre class="install-code">
1. 下载对应架构的探针程序
2. 添加执行权限: sudo chmod 777 net-manager-client-linux-x86
3. 启动探针: sudo ./net-manager-client-linux-x86 &
4. 探针启动后会在后台运行，同时自动加入开机自启动</pre
          >
        </a-typography-paragraph>
      </a-tab-pane>
...
import { WindowsOutlined, DownloadOutlined } from '@ant-design/icons-vue'

/**
 * 下载探针文件
 */
const downloadAgent = (filename) => {
  try {
    // 构建下载链接（agent 文件夹已经复制到 static 目录下）
    const downloadUrl = `/agent/${filename}`
