...
class SystemCollector:
    ...
    def get_mac_address(self):
        ...
        try:
            ...
            for interface, addrs in net_if_addrs.items():
                # 跳过回环接口
                if interface.lower().startswith("lo") or interface.lower().startswith(
                    "loopback"
                ):
                    continue
    ...
    def _get_windows_gateway(self, current_ip):
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    if self._is_valid_ip(gateway_candidate):
                        ...
                        self.logger.debug(
                            f"通过PowerShell Get-NetRoute获取到网关: {gateway}"
                        )
                        return gateway
            except Exception as e:
            ...
            try:
                result = subprocess.run(
                    ["route", "print"], capture_output=True, text=True, timeout=5
                )
                if result.returncode == 0:
                    lines = result.stdout.split("\n")
                    for line in lines:
                        ...
                        if line.strip().startswith("0.0.0.0") and "0.0.0.0" in line:
                            ...
                            if len(parts) >= 3:
                                # 检查网关是否有效
                                gateway_candidate = parts[
                                    3
                                ]  # Windows route print中网关在第4列
                                if self._is_valid_ip(gateway_candidate):
                                    gateway = gateway_candidate
                                    self.logger.debug(
                                        f"通过route print命令获取到网关: {gateway}"
                                    )
                                    return gateway
            except Exception as e:
                    ...
                    for line in lines[1:]:  # 跳过标题行
                        ...
                        if len(fields) >= 3 and fields[1] == "00000000":  # 默认路由
                            ...
                            if gateway_ip and self._is_valid_ip(gateway_ip):
                                gateway = gateway_ip
                                self.logger.debug(
                                    f"通过/proc/net/route文件获取到网关: {gateway}"
                                )
                                return gateway
            except Exception as e:
    ...
    def get_network_interfaces(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for interface_name, initial_stats in net_io_initial.items():
                ...
                # 获取接口的地址信息
                ip_address, mac_address = interface_addresses.get(
                    interface_name, ("", "")
                )
    ...
    def get_cpu_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cpu_info["physical_cores"] = (
                physical_cores if physical_cores is not None else "unknown"
            )
            ...
            try:
                ...
                if cpu_freq:
                    cpu_info["max_frequency"] = (
                        round(cpu_freq.max, 2) if cpu_freq.max else "unknown"
                    )
                    cpu_info["current_frequency"] = (
                        round(cpu_freq.current, 2) if cpu_freq.current else "unknown"
                    )
                else:
            ...
            try:
                ...
                cpu_info["load_average"] = (
                    [round(load, 2) for load in cpu_load] if cpu_load else "unknown"
                )
        ...
        try:
            ...
            try:
                ...
                memory_info["available"] = (
                    mem.available if mem.available is not None else "unknown"
                )
                memory_info["used"] = mem.used if mem.used is not None else "unknown"
                memory_info["percentage"] = (
                    round(mem.percent, 2) if mem.percent is not None else "unknown"
                )
            ...
            try:
                ...
                memory_info["swap_total"] = (
                    swap.total if swap.total is not None else "unknown"
                )
                memory_info["swap_used"] = (
                    swap.used if swap.used is not None else "unknown"
                )
                memory_info["swap_free"] = (
                    swap.free if swap.free is not None else "unknown"
                )
                memory_info["swap_percentage"] = (
                    round(swap.percent, 2) if swap.percent is not None else "unknown"
                )
            ...
            try:
                ...
                for partition in partitions:
                    try:
                        ...
                        # 权限不足时只记录debug级别日志，这是正常现象
                        self.logger.debug(
                            f"跳过分区 {partition.mountpoint}：权限不足 (需要root权限访问)"
                        )
                        continue
                    except OSError as e:
                        # 处理其他操作系统错误（如分区不可访问）
                        if e.errno == 13:  # Permission denied
                            self.logger.debug(
                                f"跳过分区 {partition.mountpoint}：权限不足 (errno 13)"
                            )
                        else:
                            self.logger.warning(f"跳过分区 {partition.mountpoint}：{e}")
                        continue
                    except Exception as e:
                        self.logger.warning(
                            f"获取分区 {partition.mountpoint} 信息失败: {e}"
                        )
                        continue
                ...
                disk_info["percentage"] = (
                    round((used_disk_space / total_disk_space) * 100, 2)
                    if total_disk_space > 0
                    else "unknown"
                )
            ...
            try:
                ...
                if disk_io:
                    disk_info["read_bytes"] = (
                        disk_io.read_bytes
                        if disk_io.read_bytes is not None
                        else "unknown"
                    )
                    disk_info["write_bytes"] = (
                        disk_io.write_bytes
                        if disk_io.write_bytes is not None
                        else "unknown"
                    )
                    disk_info["read_count"] = (
                        disk_io.read_count
                        if disk_io.read_count is not None
                        else "unknown"
                    )
                    disk_info["write_count"] = (
                        disk_io.write_count
                        if disk_io.write_count is not None
                        else "unknown"
                    )
                    disk_info["read_time"] = (
                        disk_io.read_time
                        if hasattr(disk_io, "read_time")
                        and disk_io.read_time is not None
                        else "unknown"
                    )
                    disk_info["write_time"] = (
                        disk_io.write_time
                        if hasattr(disk_io, "write_time")
                        and disk_io.write_time is not None
                        else "unknown"
                    )
            except Exception as e:
                    ...
                    try:
                        ...
                        process_info = {
                            ...
                            "memory_percent": round(
                                proc.info.get("memory_percent") or 0, 2
                            ),
            ...
            if not processes:
                try:
                    ...
                    current_proc_info = {
                        ...
                        "memory_percent": round(
                            current_process.memory_percent() or 0, 2
                        ),
                        "status": (
                            current_process.status()
                            if hasattr(current_process, "status")
                            else "unknown"
                        ),
                        "listening_ports": [],
                    }
