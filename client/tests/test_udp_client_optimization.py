...
class TestUDPClientOptimization(unittest.TestCase):
    """UDP客户端优化测试类"""
    ...
    @patch("src.network.udp_client.psutil.net_if_stats")
    @patch("src.network.udp_client.psutil.net_if_addrs")
    def test_get_active_interfaces_cached(self, mock_net_if_addrs, mock_net_if_stats):
        """测试网络接口缓存功能"""
        # 设置模拟返回值
        mock_net_if_stats.return_value = {
            "eth0": MagicMock(isup=True, is_loopback=False)
        }
        ...
        # 第一次调用
        result1 = self.udp_client._get_active_interfaces()

        # 第二次调用（应该使用缓存）
        result2 = self.udp_client._get_active_interfaces()
